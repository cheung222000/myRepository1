...
resource "azurerm_virtual_network_gateway" "gateway_hub" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.vpngateway["${each.value.vpn.gateway_traffic[0]}-${each.key}"].id              # ${each.value.vpn.gateway_traffic[0]}-${each.key} = '145.43.244.136-dataservices-nonprod-uksouth'
...
resource "azurerm_virtual_network" "myAzureVirtualNetwork2" {
  #name                = "myAzureVirtualNetwork2"
  name                = "networks-production-uksouth-vnet"
  location            = azurerm_resource_group.myAzureResourceGroup2.location
  resource_group_name = azurerm_resource_group.myAzureResourceGroup2.name
  #address_space       = ["10.0.0.0/16"]
  address_space       = ["10.241.38.0/24"]
}
