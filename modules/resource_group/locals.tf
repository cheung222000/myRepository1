locals {
  ...
  #unique_name         = var.unique_name == "true" ? random_integer.suffix[0].result : (var.unique_name == "false" ? null : var.unique_name) ##if unique_name is true then return random number, if not then see if it is false, then return null, if not then return the input.
}
